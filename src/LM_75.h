...
#pragma region PUBLIC
  ...
  void thy_w(uint16_t &thy);    // Temperature hysteresis write
  uint16_t thy_r(void);         // Temperature hysteresis read
  void tos_w(uint16_t &tos);    // Temperature over temperature shut down write
...
#pragma region PRIVATE
  ...
  uint8_t _address;
  uint8_t _config;
  int16_t _temperature_hyst;
  int16_t _temperature_os;
  uint8_t _temperature_high;
  uint8_t _temperature_low;
